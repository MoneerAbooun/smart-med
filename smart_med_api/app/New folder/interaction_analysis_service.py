...
def _clean_aliases(values: Iterable[str | None]) -> set[str]:
    ...
    for value in values:
        ...
        if normalized:
            ...
            aliases.update({part for part in normalized.split(",") if part.strip()})
    return aliases
...
def analyze_interaction(
    *,
    ...
    if direct_contra:
        return InteractionResult(
            severity="High",
            summary="One of the drug labels directly mentions the other medicine in a contraindication section.",
            mechanism="The public labeling suggests this combination should be avoided or used only with specialist guidance.",
            warnings=[
                "A direct contraindication mention was found in the public labeling.",
            ],
            recommendations=[
    ...
    if direct_interaction:
        return InteractionResult(
            severity="Moderate",
            summary="One of the drug labels directly mentions the other medicine in a drug-interactions section.",
            mechanism="The public labeling flags a named interaction that may require caution, dose adjustment, or monitoring.",
            warnings=[
                "A direct drug-interaction mention was found in the public labeling.",
            ],
            recommendations=[
    ...
    if direct_warning:
        return InteractionResult(
            severity="Moderate",
            summary="One of the labels mentions the other medicine in a warning-related section.",
            ...
            warnings=[
                "A named warning mention was found in the public labeling.",
            ],
            recommendations=[
    ...
    if first_text or second_text:
        return InteractionResult(
            ...
            summary="No direct pair-specific interaction signal was found in the public labeling sources checked for this pair.",
            mechanism="This result is based on free public labeling sources and rule-based comparison, not a dedicated clinical interaction database.",
            warnings=[
